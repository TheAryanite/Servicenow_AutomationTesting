...
test("Service now - Delete User", async ({ page }) => {
  ...
  await frame.locator('#sys_user\\.user_name').fill('Aryan.tester');
  ...
  await searchBox.fill('Aryan.tester');
  await searchBox.press('Enter');
  await page.waitForTimeout(3000);

  await newFrame.locator('a.linked.formlink').first().click();
  await page.waitForTimeout(4000);

  await frame.locator('#sysverb_delete').click();

  await frame.locator('#ok_button').click();

  await expect(frame.locator('div.list2_empty-state-list').first()).toContainText('No records to display');
  await page.waitForTimeout(5000);
