...
test("Service now - Remove Role from User", async ({ page }) => {
  ...
  await editFrame.locator('select#select_1').selectOption({ label: 'actsub_user' });
  await page.waitForTimeout(1000);

  await editFrame.locator('#remove_from_collection_button').click();
  await page.waitForTimeout(1000);

  await editFrame.locator('#select_0_sysverb_save').click();
  await page.waitForTimeout(2000);

  await expect(frame.locator('[id="sys_user.sys_user_has_role.user"] > div.list2_empty-state-list')).toBeVisible();
  await expect(frame.locator('[id="sys_user.sys_user_has_role.user"] > div.list2_empty-state-list')).toContainText('No records to display');
});